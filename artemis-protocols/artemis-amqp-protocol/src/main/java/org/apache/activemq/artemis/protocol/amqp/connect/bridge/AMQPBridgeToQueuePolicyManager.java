         ...
         queueSender.setRemoteClosedHandler((closedSender) -> {
            synchronized (this) {
               ...
               if (configuration.isLinkRecoveryEnabled() && isActive()) {
                  ...
                  AMQPBridgeSenderRecoveryHandler<AMQPBridgeToQueueEntry> recoveryHandler = queueEntry.getRecoveryHandler();
                  if (recoveryHandler == null) {
                     queueEntry.setRecoveryHandler(
                        recoveryHandler = new AMQPBridgeSenderRecoveryHandler<>(queueEntry, this::linkRecoveryHandler, configuration));
                  }
