...
import org.apache.activemq.artemis.protocol.amqp.connect.bridge.AMQPBridgeSenderController.SenderRole;
import org.apache.activemq.artemis.utils.CompositeAddress;

/**
 * Support methods for working with the AMQP bridgeManager management types
 */
public final class AMQPBridgeManagementSupport {

   // Resource names specific to bridgeManager resources that exist on the local end of a broker connection.

   /**
    * Template used to denote bridgeManager manager instances in the server management registry.
    */
   public static final String BRIDGE_MANAGER_RESOURCE_TEMPLATE = "brokerconnection.%s.bridge.%s";

   /**
    * Template used to denote bridgeManager policy managers created on the source in the server management registry.
    */
   public static final String BRIDGE_POLICY_RESOURCE_TEMPLATE = BRIDGE_MANAGER_RESOURCE_TEMPLATE + ".policy.%s";

   /**
    * Template used to denote bridgeManager receivers on the source in the server management registry. Since policy
    * names are unique on the local broker AMQP bridgeManager configuration these names should not collide as each
   ...
   /**
    * Template used to denote bridgeManager senders on the source in the server management registry. Since policy
    * names are unique on the local broker AMQP bridgeManager configuration these names should not collide as each
    ...
    * object name when adding and removing AMQP bridgeManager related control elements.
    */
   public static final String BRIDGE_NAME_TEMPLATE = "serviceCatagory=bridgeManager,bridgeName=%s";

   /**
    * The template used to create the object name suffix that is appending to the broker connection
    * object name when adding and removing AMQP bridgeManager policy control elements.
    */
   public static final String BRIDGE_POLICY_NAME_TEMPLATE = BRIDGE_NAME_TEMPLATE + ",policyType=%s,policyName=%s";

   /**
    * The template used to create the object name suffix that is appending to the broker connection
    * object name when adding and removing AMQP bridgeManager queue receiver control elements.
    */
   public static final String BRIDGE_QUEUE_RECEIVER_NAME_TEMPLATE = BRIDGE_POLICY_NAME_TEMPLATE + ",linkType=receivers,fqqn=%s";

   /**
    * The template used to create the object name suffix that is appending to the broker connection
    * object name when adding and removing AMQP bridgeManager address receiver control elements.
    */
   ...
   /**
    * The template used to create the object name suffix that is appending to the broker connection
    * object name when adding and removing AMQP bridgeManager queue sender control elements.
    */
   public static final String BRIDGE_QUEUE_SENDER_NAME_TEMPLATE = BRIDGE_POLICY_NAME_TEMPLATE + ",linkType=senders,fqqn=%s";

   /**
    * The template used to create the object name suffix that is appending to the broker connection
    * object name when adding and removing AMQP bridgeManager address sender control elements.
    */
    ...
    *
    * @param bridge
    *    The bridgeManager manager instance being registered with management.
    *
    * @throws Exception if an error occurs while registering the bridgeManager with the management services.
    */
    ...
    *
    * @param bridge
    *    The bridgeManager manager instance being unregistered from management.
    *
    * @throws Exception if an error occurs while unregistering the bridgeManager with the management services.
    */
   ...
   /**
    * Register an AMQP bridgeManager policy manager with the server management services.
    *
    * @param manager
    *    The AMQP bridgeManager policy manager instance that is being managed.
    *
   ...
   /**
    * Unregister an AMQP bridgeManager policy manager with the server management services.
    *
    * @param manager
    *    The AMQP bridgeManager policy manager instance that is being managed.
    *
   ...
   /**
    * Registers the bridgeManager receiver with the server management services on the source.
    *
    * @param receiver
   ...
   /**
    * Unregisters the bridgeManager receiver with the server management services on the source.
    *
    * @param receiver
    *    The AMQP bridgeManager receiver instance that is being managed.
    *
    * @throws Exception if an error occurs while registering the receiver with the management services.
    */
   ...
   /**
    * Registers the bridgeManager sender with the server management services on the source.
    *
    * @param sender
    *    The AMQP bridgeManager sender controller that manages the bridgeManager sender.
    *
    * @throws Exception if an error occurs while registering the producer with the management services.
    */
   public static void registerBridgeSender(AMQPBridgeSenderController sender) throws Exception {
   ...
   /**
    * Unregisters the bridgeManager sender with the server management services on the source.
    *
    * @param sender
    *    The AMQP bridgeManager sender controller that manages the bridgeManager producer.
    *
    * @throws Exception if an error occurs while registering the sender with the management services.
    */
   public static void unregisterBridgeSender(AMQPBridgeSenderController sender) throws Exception {
      final AMQPBridgePolicyManager manager = sender.getPolicyManager();
      final AMQPBridgeManager bridgeManager = manager.getBridgeManager();
      final String brokerConnectionName = bridgeManager.getBrokerConnection().getName();
      final ActiveMQServer server = bridgeManager.getServer();
