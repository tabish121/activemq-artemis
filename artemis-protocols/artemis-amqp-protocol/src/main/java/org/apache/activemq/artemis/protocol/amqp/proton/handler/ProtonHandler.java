...
public class ProtonHandler extends ProtonInitializable implements SaslListener {
   ...
   private static final int TRY_LOCK_SPIN_COUNT = 10;
   private static final int TRY_LOCK_YIELD_COUNT = 70;

   private boolean tryLockForInput() {
      int idleCount = 0;

      while (!lock.tryLock()) {
         if (idleCount < TRY_LOCK_SPIN_COUNT) {
            idleCount++;
         } else if (idleCount < TRY_LOCK_YIELD_COUNT) {
            Thread.yield();
            idleCount++;
         } else {
            return false;
