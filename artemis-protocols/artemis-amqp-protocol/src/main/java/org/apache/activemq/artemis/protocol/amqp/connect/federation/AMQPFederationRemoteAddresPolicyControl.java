 ...
 */
public class AMQPFederationRemoteAddresPolicyControl extends AbstractControl implements AMQPFederationRemotePolicyControl {

   private final AMQPFederationRemoteAddressPolicyManager manager;

   public AMQPFederationRemoteAddresPolicyControl(ActiveMQServer server, AMQPFederationRemoteAddressPolicyManager manager) throws NotCompliantMBeanException {
   ...
   public long getMessagesSent() {
      if (AuditLogger.isBaseLoggingEnabled()) {
         AuditLogger.getMessagesReceived(manager);
      }
      clearIO();
      try {
         return manager.getMessagesSent();
      } finally {
         blockOnIO();
      }
   }
