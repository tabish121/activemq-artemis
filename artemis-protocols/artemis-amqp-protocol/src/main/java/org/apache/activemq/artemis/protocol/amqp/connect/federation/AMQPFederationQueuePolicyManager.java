...
public final class AMQPFederationQueuePolicyManager extends AMQPFederationLocalPolicyManager implements ActiveMQServerConsumerPlugin, ActiveMQServerBindingPlugin {
    ...
    * configuration is reloaded and this federation instance happens to be updated but not removed.
    */
   private final String POLICY_MANAGER_ATTACHMENT_KEY = UUID.randomUUID().toString();
   ...
   public synchronized void afterCloseConsumer(ServerConsumer consumer, boolean failed) {
      if (isActive()) {
         final FederationConsumerInfo consumerInfo = (FederationConsumerInfo) consumer.getAttachment(POLICY_MANAGER_ATTACHMENT_KEY);
   ...
   private void reactIfConsumerMatchesPolicy(ServerConsumer consumer) {
      ...
      if (testIfQueueMatchesPolicy(consumer.getQueueAddress().toString(), queueName)) {
         ...
         if (federationConsumers.containsKey(consumerInfo)) {
            ...
            federationConsumers.put(consumerInfo, entry = new AMQPFederationQueueConsumerManager(this, POLICY_MANAGER_ATTACHMENT_KEY, consumerInfo, consumer.getQueue()));
         }
