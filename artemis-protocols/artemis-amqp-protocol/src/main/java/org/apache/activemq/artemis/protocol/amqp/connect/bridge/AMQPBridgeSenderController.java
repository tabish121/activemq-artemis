 ...
 */
public abstract class AMQPBridgeSenderController implements SenderController {
   ...
   protected final AMQPBridgeToPolicyManager manager;
   ...
   protected final Consumer<AMQPBridgeSenderController> closedListener;

   public AMQPBridgeSenderController(AMQPBridgeToPolicyManager manager, AMQPSessionContext session, SenderMetrics metrics, Consumer<AMQPBridgeSenderController> closedListener) throws ActiveMQAMQPException {
      this.manager = manager;
      this.bridge = manager.getBridgeManager();
   ...
   public final void close(boolean remoteClose) throws Exception {
      try {
         if (bridge != null) {
            bridge.removeLinkClosedInterceptor(controllerId);
         }
   ...
   protected final void registerRemoteLinkClosedInterceptor(Sender protonSender) {
      Objects.requireNonNull(bridge, "Initialization should have validated bridge state before adding an interceptor");

      bridge.addLinkClosedInterceptor(controllerId, (link) -> {
         // Normal close from remote due to demand being removed is handled here but remote close with an error is left
         // to the parent bridge instance to decide on how it should be handled.
