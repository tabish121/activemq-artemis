...
/**
 * Base class for AMQP Federation consumer that aggregates some of the common APIs
 * into a generic base.
 */
public abstract class AMQPFederationConsumer implements FederationConsumerInternal {
   ...
   protected final BiConsumer<FederationConsumerInfo, Message> incomingMessageSpy;
   ...
   public AMQPFederationConsumer(AMQPFederation federation, AMQPFederationConsumerConfiguration configuration,
                                 ...
                                 BiConsumer<FederationConsumerInfo, Message> incomingMessageSpy) {
      this.federation = federation;
      this.consumerInfo = consumerInfo;
      this.connection = session.getAMQPConnectionContext();
      this.session = session;
      this.configuration = configuration;
      this.incomingMessageSpy = incomingMessageSpy;
   }

   /**
    * @return the number of messages this consumer has read from the remote during its lifetime.
    */
    ...
    */
   protected final void recordFederatedMessageReceopt(Message message) {
      messageCount.incrementAndGet();

      if (incomingMessageSpy != null) {
         incomingMessageSpy.accept(consumerInfo, message);
      }
   }
