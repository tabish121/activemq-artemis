 ...
 */
public class AMQPBridgeToQueueSender implements AMQPBridgeSender {
   ...
   private void asyncCreateSender() {
      connection.runLater(() -> {
         ...
         try {
            ...
            final Symbol[] targetCapabilities = policy.getRemoteTerminusCapabilities().toArray(new Symbol[0]);
            ...
            protonSender.setSenderSettleMode(policy.isPresettle() ? SenderSettleMode.SETTLED : SenderSettleMode.UNSETTLED);
            ...
            protonSender.attachments().set(AMQP_LINK_INITIALIZER_KEY, Runnable.class, () -> {
               try {
                  ...
                  bridge.addLinkClosedInterceptor(senderInfo.getId(), remoteCloseInterceptor);
