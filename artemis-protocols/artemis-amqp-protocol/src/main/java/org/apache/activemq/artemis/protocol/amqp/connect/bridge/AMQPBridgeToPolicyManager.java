...
public abstract class AMQPBridgeToPolicyManager extends AMQPBridgePolicyManager implements ActiveMQServerBasePlugin {
   ...
   protected void handleManagerInitialized() {
      ...
      try {
         // TODO: bridge.registerLocalPolicyManagement(this);
      } catch (Exception e) {
   ...
   protected void handleManagerShutdown() {
      ...
      try {
         // TODO: bridge.unregisterLocalPolicyManagement(this);
      } catch (Exception e) {
         logger.trace("Error while attempting to remove sender policy control to management", e);
      }
   ...
   /**
    * Attempts to close a bridge sender. The method will not double close a sender as it
    * checks the closed state. The method is synchronized to allow for use in asynchronous
    * call backs from bridge senders.
    *
    * @param bridgeSender
    *    A bridge sender to close, or null in which case no action is taken.
    */
   protected synchronized void tryCloseBridgeSender(AMQPBridgeSender bridgeSender) {
      if (bridgeSender != null) {
         try {
            if (!bridgeSender.isClosed()) {
               bridgeSender.close();
            }
         } catch (Exception ignore) {
            logger.trace("Caught error on attempted close of existing bridge sender", ignore);
         }
      }
   }
}
