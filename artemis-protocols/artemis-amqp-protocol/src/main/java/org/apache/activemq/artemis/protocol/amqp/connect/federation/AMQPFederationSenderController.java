...
public abstract class AMQPFederationSenderController implements SenderController {
   ...
   public AMQPFederationSenderController(AMQPFederationRemotePolicyManager manager, ProducerMetrics metrics, AMQPSessionContext session) throws ActiveMQAMQPException {
   ...
   /**
    * Called on initialization of sender either to trigger addition of the sender management
    * beans to the registry.
    */
   protected abstract void registerSenderManagement();

   /**
    * Called on close of sender either from the local or the remote side to remove the
    * management beans from the registry.
    */
   protected abstract void unregisterSenderManagement();
   ...
   private static class CountedMessageWrites implements MessageWriter {
      ...
      public void accept(MessageReference messageReference) {
         wrapped.accept(messageReference);
      }
   }
}
