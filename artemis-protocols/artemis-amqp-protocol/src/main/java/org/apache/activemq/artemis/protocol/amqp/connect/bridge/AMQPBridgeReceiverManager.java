 ...
 */
public abstract class AMQPBridgeReceiverManager {
   ...
   /**
    * Attempt to recover a stopped receiver if this manager is not in the closed
    * state and there is active demand registered. When in the stopped state the
    * existing receiver will be restarted and if no active receiver exists then a
    * new receiver is created. If this is called while a receiver is in the started
    * state then this operation is a no-op and the receiver is left as is.
    *
    * The bridge manager should be calling this method with its lock held.
    */
   public void recover() {
      ...
      if (hasDemand()) {
         ...
         // action is needed such as restarting or recreating the receiver.
    ...
    */
   public void removeDemand(String demand) {
      checkClosed();
