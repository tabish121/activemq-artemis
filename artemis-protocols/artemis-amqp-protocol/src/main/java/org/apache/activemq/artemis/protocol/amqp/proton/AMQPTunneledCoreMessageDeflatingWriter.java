...
public class AMQPTunneledCoreMessageDeflatingWriter implements MessageWriter {
   ...
   private final byte[] scratchBuffer = new byte[2048];
   ...
   protected int deflateEncoding(ByteBuf inputBuf, ByteBuf outputBuf) {
      ...
      while (!deflater.finished()) {
         compressedBytes += deflateResult = deflater.deflate(scratchBuffer);
         outputBuf.writeBytes(scratchBuffer, 0, deflateResult);
      }
