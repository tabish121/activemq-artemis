 ...
 */
public class AMQPBridgeFromQueueReceiver implements AMQPBridgeReceiver {
   ...
   private AMQPBridgeQueueDeliveryReceiver receiver;
   ...
   @Override
   public synchronized AMQPBridgeFromQueueReceiver setRemoteOpenHandler(Consumer<AMQPBridgeReceiver> handler) {
      if (started) {
   ...
   @Override
   public synchronized AMQPBridgeFromQueueReceiver setRemoteClosedHandler(Consumer<AMQPBridgeReceiver> handler) {
      if (started) {
   ...
   protected boolean remoteLinkClosedInterceptor(Link link) {
      if (link == protonReceiver && link.getRemoteCondition() != null && link.getRemoteCondition().getCondition() != null) {
         ...
         logger.debug("AMQP Bridge {} queue receiver {} closed with condition: {}", bridge.getName(), receiverInfo, errorCondition);
   ...
   private void asyncCreateReceiver() {
      connection.runLater(() -> {
         ...
         try {
            ...
            // TODO: Support tunneling on bridge receivers ?
            ...
            // core tunneling is however currently Artemis won't enable this for generic AMQP
            // links but might in the future? Or there could be some identifying property for
            // bridge receivers that allowed the remote to detect and react accordingly?
            if (configuration.isCoreMessageTunnelingEnabled()) {
               protonReceiver.setOfferedCapabilities(new Symbol[] {CORE_MESSAGE_TUNNELING_SUPPORT});
            }
