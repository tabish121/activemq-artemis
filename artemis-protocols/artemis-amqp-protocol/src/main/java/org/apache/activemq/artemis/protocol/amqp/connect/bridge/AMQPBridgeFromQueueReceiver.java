   ...
   private void asyncCreateReceiver() {
      connection.runLater(() -> {
         ...
         try {
            ...
            final Symbol[] sourceCapabilities = policy.getRemoteTerminusCapabilities().toArray(new Symbol[0]);
            ...
            protonReceiver.setSenderSettleMode(policy.isPresettle() ? SenderSettleMode.SETTLED : SenderSettleMode.UNSETTLED);
            ...
            protonReceiver.attachments().set(AMQP_LINK_INITIALIZER_KEY, Runnable.class, () -> {
               try {
                  ...
                  bridge.addLinkClosedInterceptor(receiverInfo.getId(), remoteCloseInterceptor);
