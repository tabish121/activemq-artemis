 ...
 * with differing filters might still have overlaps in the messages they receive then more message traffic cross the
 * federation than would be generated if using the standard conduit consumer pattern (this can also happen if filtered
 * and non-filtered consumers are mixed on the local address).
 */
public final class AMQPFederationAddressBindingsConsumer extends AMQPFederationAddressConsumer {
   ...
   public AMQPFederationAddressBindingsConsumer(AMQPFederationAddressPolicyManager manager,
      ...
      this.storageManager = manager.getFederation().getServer().getStorageManager();
   }
         ...
         try {
            ...
            storageManager.afterCompleteOperations(new IOCallback() {
               ...
               public void onError(int errorCode, String errorMessage) {
                  logger.warn("onError after IO completion{}-{}", errorCode, errorMessage);
               }
            });
         } catch (Exception e) {
