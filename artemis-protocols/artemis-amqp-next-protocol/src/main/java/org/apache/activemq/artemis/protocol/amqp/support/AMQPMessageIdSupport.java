 ...
 * encoded binary, UUID, or unsigned long but can't be converted into the indicated
 * format, an exception will be thrown.
 *
 */
public class AMQPMessageIdSupport {

   public static final AMQPMessageIdSupport INSTANCE = new AMQPMessageIdSupport();
   ...
   public String toMessageIdString(Object idObject) {
      if (idObject instanceof String) {
         ...
         final boolean hasMessageIdPrefix = hasMessageIdPrefix(stringId);
   ...
   private String convertToIdString(Object idObject) {
      ...
      if (idObject instanceof UUID) {
         ...
         return JMS_ID_PREFIX + AMQP_BINARY_PREFIX + convertBinaryToHexString(((Binary) idObject).asByteArray());
      } else {
   ...
   public Object toIdObject(final String origId) throws ActiveMQAMQPIllegalStateException {
      ...
      if (!AMQPMessageIdSupport.INSTANCE.hasMessageIdPrefix(origId)) {
   ...
   public byte[] convertHexStringToBinary(String hexString) throws IllegalArgumentException {
      ...
      final byte[] binary = new byte[length / 2];

      for (int i = 0; i < length; i += 2) {
         final char highBitsChar = hexString.charAt(i);
         final char lowBitsChar = hexString.charAt(i + 1);

         final int highBits = hexCharToInt(highBitsChar, hexString) << 4;
         final int lowBits = hexCharToInt(lowBitsChar, hexString);
