...
public class AMQPBrokerConnectionSenderTest extends AmqpClientTestSupport {
   ...
   public void testCreatesSenderWithConfiguredTargetAddressOnRemote() throws Exception {
      doTestCreatesSenderOnRemote("test", "topic://", "topic://test", null);
   }

   @Test
   @Timeout(20)
   public void testCreatesSenderWithConfiguredTargetElementsOnRemote() throws Exception {
      doTestCreatesSenderOnRemote("test", "topic://", "topic://test", "topic");
   }

   @Test
   @Timeout(20)
   public void testCreatesSenderMultipleTargetCapabilitiesOnRemote() throws Exception {
      doTestCreatesSenderOnRemote("test", "queue://", "queue://test", "queue,temp,test");
   }

   private void doTestCreatesSenderOnRemote(String address, String targetAddressPrefix, String expectedTargetAddress, String targetCapabilities) throws Exception {
      try (ProtonTestServer peer = new ProtonTestServer()) {
         peer.expectSASLAnonymousConnect();
         peer.expectOpen().respond();
         peer.expectBegin().respond();
         ...
         if (targetAddressPrefix != null) {
            element.setTargetAddressPrefix(targetAddressPrefix);
         }
